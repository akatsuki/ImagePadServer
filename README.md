...
- 動画サイト URL や SoundCloud トラック URL を yt-dlp で取得して HLS として再配信する
- 履歴から画像・動画を再公開する
- よく使う画像・動画をお気に入りに残す
- 動画変換キューとサムネイルを表示する
...
SoundCloud トラックの HLS は、音声が AAC stereo 48kHz、映像が H.264 / yuv420p で出力されます。波形は FFmpeg の `showwaves` フィルターが音源ファイルを解析して 30fps の映像フレームとして焼き込みます。
...
- 起動時と終了時にメディアワークスペースを初期化する（前回の画像・動画・HLS は次回起動まで残らない）
