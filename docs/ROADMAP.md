...
- OBS 遅延モード（HLS / LHLS / LL-HLS / RTSPT）
  - 4 方式から選択。LHLS は community LHLS（DASH muxer → ループバック HTTP sink）、LL-HLS と RTSPT はアプリ所有の MediaMTX サイドカー経由。
  - LHLS と LL-HLS は **実験的**（ブラウザ／VRChat 受け入れ確認まで）。RTSPT は PC 専用で `rtspt://` URL を配布。
  - 準備が整うまで公開せず、別方式への無言フォールバックはしない。設定値から実測遅延は断定しない。
  - 関連コード: `internal/obsrtmp/lhls_sink.go`, `internal/obsrtmp/mediamtx.go`, `internal/obsrtmp/manager.go`
  - 詳細: `docs/OBS_AVPRO_FEASIBILITY.md`, `docs/OBS_LATENCY_PROTOCOL_DECISION.md`
