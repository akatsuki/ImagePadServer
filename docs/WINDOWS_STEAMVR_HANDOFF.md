...
- `maxDimension`: default/max `2048`
- `maxMB`: default/max `30`
...
- Prefer the public `imageURL`.
- Offer local URL only as a secondary/fallback option.
...
3. Add `internal/steamvr` no-op package and app hook.
4. Investigate OpenVR overlay feasibility.
...
7. Add URL copy:
   - Copy `imageURL` from `/api/state` to Windows clipboard
