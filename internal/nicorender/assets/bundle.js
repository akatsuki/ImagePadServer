...
(function(global, factory) {
	...
	*/
	...
	new RegExp("^(?:[\\u{1F1E6}-\\u{1F1FF}]{2}|\\u{1F3F4}[\\u{E0061}-\\u{E007A}]{2}[\\u{E0030}-\\u{E0039}\\u{E0061}-\\u{E007A}]{1,3}\\u{E007F}|(?:\\p{Emoji}\\uFE0F\\u20E3?|\\p{Emoji_Modifier_Base}\\p{Emoji_Modifier}?|(?![\\p{Emoji_Modifier_Base}\\u{1F1E6}-\\u{1F1FF}])\\p{Emoji_Presentation})(?:\\u200D(?:\\p{Emoji}\\uFE0F\\u20E3?|\\p{Emoji_Modifier_Base}\\p{Emoji_Modifier}?|(?![\\p{Emoji_Modifier_Base}\\u{1F1E6}-\\u{1F1FF}])\\p{Emoji_Presentation}))*)+$", "u");
	/* @__NO_SIDE_EFFECTS__ */
	...
	*/
	...
	const getDefaultCollisionLayer = (owner) => owner ? -2 : -1;
	const ZCommentLayer = /* @__PURE__ */ union([
		/* @__PURE__ */ literal(-1),
		/* @__PURE__ */ literal(-2),
		rangedNumber({ min: 0 })
	]);
	...
	const ZFormattedCommentEntries = /* @__PURE__ */ object({
		...
		layer: /* @__PURE__ */ optional(ZCommentLayer),
		ignoreScale: /* @__PURE__ */ optional(/* @__PURE__ */ boolean(), false),
		is_my_post: /* @__PURE__ */ optional(/* @__PURE__ */ boolean(), false)
	});
	const ZFormattedComment = /* @__PURE__ */ pipe(ZFormattedCommentEntries, /* @__PURE__ */ transform((input) => {
		var _input$layer;
		const layer = (_input$layer = input.layer) !== null && _input$layer !== void 0 ? _input$layer : -1;
		return _objectSpread2(_objectSpread2({}, input), {}, { layer: input.owner && layer === -1 ? -2 : layer });
	}));
	/**
	* @deprecated
	*/
	const ZFormattedLegacyComment = /* @__PURE__ */ omit(ZFormattedCommentEntries, [
		"layer",
		"ignoreScale",
		"user_id",
		"is_my_post"
	...
	*/
	...
	var typeGuard_exports = /* @__PURE__ */ __exportAll({
		MAX_OPTION_SCALE: () => 8,
		default: () => typeGuard
	});
	...
	*/
	...
	const RE_BUTTON_CONTENT = new RegExp("^(?:(?<before>.*?)\\[)?(?<body>.*?)(?:\\](?<after>[^\\]]*?))?$", "su");
	const RE_LONG = /^[@＠]([0-9.]+)/;
	...
	const MAX_NICOSCRIPT_LONG = 36e4;
	...
	const addMovableCollisionCommentToBuckets = (index, comment) => {
	...
	const flushPendingMovableCollisionComments = (index) => {
		if (index.pending.length === 0) return;
		for (const comment of index.pending) addMovableCollisionCommentToBuckets(index, comment);
		index.pending = [];
	};
	...
	*/
	const parseCommand = (comment, _command, result, isFlash, config) => {
		...
		if (typeof scale === "number") {
			var _result$commandScale;
			(_result$commandScale = result.commandScale) !== null && _result$commandScale !== void 0 || (result.commandScale = scale);
			return;
		}
	...
	*/
	...
	const getScale = (match) => {
		if (!match) return;
		const value = Number(match[1]);
		if (Number.isFinite(value) && value >= Number.MIN_VALUE && value <= 8) return value;
	};
	/**
	* コメントがFlash適用対象化判定返す
	...
	const getPosY = (_currentPos, targetComment, collision, config) => {
		...
		restart: while (true) {
			for (let i = 0; i < len; i++) {
				...
				if (item.layer === targetLayer && currentPos < item.posY + item.height && currentPos + targetHeight > item.posY) {
	...
	*/
	const changeCALayer = (rawData, config) => {
		...
		const commentArts = filteredComments.filter((comment) => {
		...
		const commentArtsGroupedByTimes = groupCommentsByTime(commentArtsGroupedByUser, config);
	...
	*/
	...
	const getCommentArtDuplicateKey = (comment) => {
		const normalizedMail = Array.from(new Set(comment.mail.filter((mail) => !RE_CA_FILTER.test(mail)))).sort((a, b) => a.localeCompare(b));
	...
	const parseMultiFontFullWidthPart = (part, index, lineContent, config, maxItems = MAX_FLASH_CONTENT_ITEMS) => {
		...
		if (config.flashMode === "xp") {
			...
			for (let i = 1, n = index.length; i < n; i++) {
				...
				addPartToResult(lineContent, content, config, getFlashFontName(lastVal.font), maxItems);
				offset = currentVal.index;
			}
			const val = index[index.length - 1];
			if (val) {
				const content = part.slice(offset);
				addPartToResult(lineContent, content, config, getFlashFontName(val.font), maxItems);
			}
			return;
		}
	...
	*/
	var BaseComment = class {
		...
		*/
		...
		getLayerScale(parsedData) {
			var _parsedData$commandSc;
			return (parsedData.ignoreScale ? 1 : this.ctx.options.scale) * ((_parsedData$commandSc = parsedData.commandScale) !== null && _parsedData$commandSc !== void 0 ? _parsedData$commandSc : 1);
		}
		get owner() {
	...
	*/
	...
	var FlashComment = class extends BaseComment {
		...
		*/
		getCommentSize(parsedData) {
			...
			const meas = this.measureText(_objectSpread2(_objectSpread2({}, parsedData), {}, {
				scale: 1,
				layerScale
			}));
			if (layerScale !== 1) {
		...
		parseContent(input, button) {
			...
			const displayText = button ? `${button.message.before}${button.message.body}${button.message.after}` : input;
		...
		measureText(comment) {
			...
			const drawScale = this._globalScale * comment.layerScale;
			...
			if (showCollision) {
				...
				try {
					...
					for (let i = 0, n = this.comment.lineCount; i < n; i++) {
						...
						this.renderer.strokeRect(posX, posY + linePosY * this._globalScale, this.comment.width, this.comment.fontSize * this.comment.lineHeight * -1 * this._globalScale * this.comment.scale * this.comment.layerScale);
					}
				} finally {
					this.renderer.restore();
				}
		...
		isHovered(_cursor, _posX, _posY) {
			...
			const scaleY = this._globalScale * this.comment.scale * this.comment.layerScale;
		...
		_setupCanvas(renderer) {
			...
			const scale = this._globalScale * this.comment.scale * this.comment.layerScale;
	...
	const MAX_HTML5_COMMENT_IMAGE_WIDTH = 8184;
	const MAX_HTML5_COMMENT_IMAGE_HEIGHT = 8184;
	...
	var HTML5Comment = class extends BaseComment {
		...
		_generateTextImage() {
			...
			const paddingTop = (10 - scale * 10) * ((this.comment.lineCount + 1) / this.config.html5HiResCommentCorrection);
			const drawScale = getConfig(this.config.commentScale, false) * scale * this.comment.layerScale;
	...
	*/
	const fromLegacy = (data) => {
		...
		for (const _val of data) {
			const chat = typeof _val === "object" && _val !== null && "chat" in _val ? _val.chat : void 0;
			const val = /* @__PURE__ */ safeParse(ZApiChat, chat);
	...
	*/
	...
	const MAX_DUPLICATE_CANVAS_CLONE_BYTES_PER_FRAME = 536870912;
