...
func TestMotionScoreClassifiesLowMotion(t *testing.T) {
	score := MotionScore{AverageFrameSize: 10 * 1024, StdDev: 2000, FrameCount: 30}
...
func TestMotionScoreClassifiesHighMotion(t *testing.T) {
	score := MotionScore{AverageFrameSize: 200 * 1024, StdDev: 100000, FrameCount: 30}
	if score.IsLowMotion() {
...
func TestAdaptPresetForContentBoostsLowMotion(t *testing.T) {
	...
	score := MotionScore{AverageFrameSize: 8 * 1024, StdDev: 1500, FrameCount: 30}
...
func TestAdaptPresetForContentClampsCRF(t *testing.T) {
	...
	score := MotionScore{AverageFrameSize: 1024, StdDev: 100, FrameCount: 30}
	adapted := AdaptPresetForContent(base, score)
	if adapted.CRF > 40 {
