...
func RenderRadioTrack(ctx context.Context, outDir, ffmpeg string, input AudioRenderInput, trackID string, preset QualityPreset, progress func(float64)) (string, error) {
	...
	if executable == "" {
		// CPU composition is retained only as a reference implementation during
		// migration. Production render jobs are GPU-only and must fail closed
		// when the sidecar has not been provisioned.
		return "", ErrGPURequired
	}
	return renderRadioTrackGPU(ctx, outDir, ffmpeg, executable, input, trackID, preset, progress)
}

// renderRadioTrackGPU feeds wgpu-produced BT.709 limited YUV420 frames directly
// to FFmpeg's existing H.264/AAC mux path. Audio remains the source file; visuals are
...
func renderRadioTrackGPU(ctx context.Context, outDir, ffmpeg, sidecarExe string, input AudioRenderInput, trackID string, preset QualityPreset, progress func(float64)) (string, error) {
	...
	args := []string{"-hide_banner", "-loglevel", "error", "-f", "rawvideo", "-pix_fmt", "yuv420p", "-s", fmt.Sprintf("%dx%d", width, height), "-r", "30", "-color_range", "tv", "-colorspace", "bt709", "-color_primaries", "bt709", "-color_trc", "bt709", "-i", "pipe:0", "-i", input.SourcePath, "-map", "0:v:0", "-map", "1:a:0", "-t", strconv.FormatFloat(input.Analysis.Duration, 'f', 3, 64)}
	args = append(args, encoder.FFmpegArgs(preset, "veryfast")...)
	args = append(args, "-pix_fmt", "yuv420p", "-color_range", "tv", "-colorspace", "bt709", "-color_primaries", "bt709", "-color_trc", "bt709", "-c:a", "aac", "-ar", "48000", "-ac", "2", "-movflags", "+faststart", outPath)
	cmd := exec.CommandContext(ctx, ffmpeg, args...)
	hideWindow(cmd)
	...
	frames := canonicalMusicVideoFrameCount(input.Analysis)
	waveW := int(math.Round(752 * float64(width) / 1280))
	pcmFrames := make(chan []uint16, 2)
	pcmErr := make(chan error, 1)
	pcmCtx, pcmCancel := context.WithCancel(ctx)
	defer pcmCancel()
	go func() {
		err := StreamWaveformPCMHistoryFrames(pcmCtx, ffmpeg, input, waveW, frames, func(_ int, samples []uint16) error {
			select {
			case pcmFrames <- samples:
				return nil
			case <-pcmCtx.Done():
				return pcmCtx.Err()
			}
	...
	for i := 0; i < frames; i++ {
		...
		var ok bool
		scene.Feature.WaveformQ16, ok = <-pcmFrames
		if !ok {
			_ = cmd.Process.Kill()
			if e := <-pcmErr; e != nil {
		...
		frame, err := sidecar.RenderSceneYUV(ctx, uint32(width), uint32(height), uint64(i), ptsNS, &scene)
		if err != nil {
			_ = cmd.Process.Kill()
			return "", fmt.Errorf("%w: render frame: %v", ErrGPURendererUnavailable, err)
		}
		packed, err := frame.PackedBytes()
		if err != nil {
			_ = cmd.Process.Kill()
			return "", fmt.Errorf("%w: pack frame: %v", ErrGPURendererUnavailable, err)
		}
		if err := writeGPUFrame(ctx, inPipe, packed); err != nil {
			_ = cmd.Process.Kill()
			return "", err
		}
		if progress != nil {
